...
def evaluate(llm_func):
    ...
    questions, expected_anss = [a["question"] for a in data], [
        a["answer"] for a in data
    ]
...
def _compare_individual_answers(expected_ans, actual_ans):
    return {
        "lev_distance": _find_levenshin_distance(expected_ans, actual_ans),
        "embed_distance": _find_embedding_distance(expected_ans, actual_ans),
    }
...
def _find_embedding_distance(expected_ans, actual_ans):
    ...
    return cosine_similarity(
        embeddings.embed_query(expected_ans), embeddings.embed_query(actual_ans)
    )
