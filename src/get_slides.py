...
def get_slides_data(filename, pages_NOT_to_include: List[str] = [1]):
    ...
    elements = partition_pdf(
        filename, infer_table_structure=False, include_page_breaks=True
    )
    ...
    for el in elements:
        ...
        if (
            elem_text in pages_NOT_to_include_str
            and elem_metadata_type == "UncategorizedText"
        ):
            ignore_page = True
