...
def index_artifacts_to_db(collection_name = "default", verbose = True):
    ...
    if verbose:
        print(f"The number of text docs retrieved is {len(chunked_docs)} with total number of token chunks being {sum([len(a) for a in chunked_tokens])}")
    collection = _create_index_from_token_chunk_metadata_pairs(chunked_tokens, collection_name)
    if verbose:
...
def retrieve_relevant_docs_from_index(queries, collection_name = "default", n_docs = 5, re_ranker_policy = "rrf", verbose = True):
    client = chromadb.PersistentClient("../assets/index")
    collection = client.get_collection(collection_name)
    
    results = collection.query(query_texts=queries, n_results=15) ## retrieve the set of results using multiple queries.
    original_query = queries[0] ## the first entry of queries is the original query.

    re_ranked_results = []
    if re_ranker_policy != "rrf":
        re_ranked_results = _re_rank_results_using_cross_encoder(results, original_query, n_docs)
    else:    
...
def _prepare_document_chunks_from_artifacts(doc_chunk_size = 512):
    ...
    chunked_doc_pairs = [[character_splitter.split_text(text_doc_pair[0]), text_doc_pair[1]] for text_doc_pair in text_docs]
    return chunked_doc_pairs


def _convert_text_chunks_into_token_chunk_metadata_pairs(chunked_doc_pairs, character_overlap = 0, tokens_per_token_chunk = 128):
    token_splitter = SentenceTransformersTokenTextSplitter(chunk_overlap=character_overlap, tokens_per_chunk=tokens_per_token_chunk)

    chunked_token_pairs = []
    for chunked_doc_pair in chunked_doc_pairs:
        ## each doc_pair consists of [List[str], str].
        for chunk_doc_pair in chunked_doc_pair:
            chunked_tokens = token_splitter.split_text(chunk_doc_pair[0]) ## splitting the document that is character splitted into token chunks based on model capacity and information specificity requirements.
            chunked_token_pairs += [[a, chunked_doc_pair[1]] for a in chunked_tokens]

    return chunked_token_pairs
    
    
def _create_index_from_token_chunk_metadata_pairs(token_metadata_pairs, collection_name):
    ...
    collection.add(ids = ids, documents = [a[0] for a in token_metadata_pairs], metadatas = [{"source": a[1]} for a in token_metadata_pairs])
    return collection
...
def _re_rank_results_using_cross_encoder(results, original_query, top_n):
    ...
    cross_encoder = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-6-v2') ## stored in local cache of system after first time run.
